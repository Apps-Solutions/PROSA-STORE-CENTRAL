		...
		function TMForm() {
			...
			function init() {
				form.on('submit', formSubmit).on('reset', formReset).on('focus', '[data-constraints]', function() {
					...
					var $this = $(this), next = $this.parents('label').next('label').find('input,textarea')
					if (e.keyCode === 13)
						if (next.length)
							next.focus()
						else
			...
			function fieldValidate(el) {
				var el = $(this), result = el.regula('validate'), isEmpty = false, isInvalid = false, isRequired = !!~el.data('constraints').indexOf('@Required')
...
(function($) {
	$.fn.TMPlaceholder = function(opt) {
		return this.each(function() {
			var th = $(this), placeholder_text, placeholder
